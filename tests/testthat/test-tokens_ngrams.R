...
test_that("test `tokens_ngrams` on characters", {
    ngms <- quanteda.core:::tokens_ngrams.character(c('insurgents', 'killed', 'in', 'ongoing', 'fighting'))
    ...
    expect_warning(quanteda.core:::tokens_ngrams.character('insurgents killed', 'in', 'ongoing', 'fighting'), 
                 "whitespace detected: you may need to run tokens\\(\\) first")
    
    expect_warning(quanteda.core:::tokens_ngrams.character(c('insurgents killed in ongoing fighting'), n = 1, skip = 1), 
