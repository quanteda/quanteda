test_that("dfm_sample works as expected", {
    ...
    expect_error(suppressWarnings(dfm_sample(mt, margin = "features", size = 3500)),
...
test_that("dfm_sample for features works as expected", {
    ...
    expect_identical({
        set.seed(100)
        suppressWarnings(as.matrix(dfm_sample(dfmat, size = 2, margin = "features", replace = TRUE)))
    }, mat1)
    ...
    expect_identical({
        set.seed(100)
        suppressWarnings(as.matrix(dfm_sample(dfmat, size = 3, margin = "features", replace = FALSE)))
    }, mat2)
})
...
test_that("dfm_sample default size arguments work as expected", {
    ...
    expect_identical({
        set.seed(100)
        suppressWarnings(as.matrix(dfm_sample(dfmat, margin = "features", replace = TRUE)))
    }, mat2)
})
