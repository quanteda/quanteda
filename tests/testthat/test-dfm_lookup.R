...
test_that("dfm_lookup with show_unmatched works", {
    ...
    expect_equivalent(
        ...
        dfm_lookup(dfm1, dict, show_unmatched = TRUE)[, 1:2]
    )
    expect_equal(
        as.matrix(dfm_lookup(dfm1, dict, show_unmatched = TRUE)),
        matrix(c(1,3,0,2,3,3), nrow = 2, dimnames = list(docs = c("d1", "d2"), features = c("one", "two", "_unmatched")))
    )
    expect_warning(
        dfm_lookup(dfm1, dict, show_unmatched = TRUE, exclusive = FALSE),
        "show_unmatched only applies if exclusive = TRUE"
    )
})
