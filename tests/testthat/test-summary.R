...
test_that("summary chache is updated", {
    ...
    summ_corp1 <- summary(corp, cache = TRUE, tolower = FALSE, remove_punct = FALSE)
    summ_corp2 <- summary(corp, cache = TRUE, tolower = TRUE, remove_punct = FALSE)
    summ_corp3 <- summary(corp, cache = TRUE, tolower = TRUE, remove_punct = TRUE)
    ...
    summ_toks1 <- summary(toks, cache = TRUE, tolower = FALSE, remove_punct = FALSE)
    summ_toks2 <- summary(toks, cache = TRUE, tolower = TRUE, remove_punct = FALSE)
    summ_toks3 <- summary(toks, cache = TRUE, tolower = TRUE, remove_punct = TRUE)
    expect_true(all(summ_toks1$n_type > summ_toks2$n_type))
    expect_true(all(summ_toks2$n_type > summ_toks3$n_type))
    toks <- tokens_remove(toks, stopwords("en"))
    summ_toks4 <- summary(toks, cache = TRUE)
    ...
    summ_dfm1 <- summary(dfmt, cache = TRUE, tolower = FALSE, stem = FALSE)
    summ_dfm2 <- summary(dfmt, cache = TRUE, tolower = TRUE, stem = FALSE)
    summ_dfm3 <- summary(dfmt, cache = TRUE, tolower = TRUE, stem = TRUE)
    ...
    summ_dfm4 <- summary(dfmt, cache = TRUE)
