...
test_that("resample works with sizes", {
    ...
    tmp <- quanteda:::resample(1:10, group = grvec, size = 2, replace = TRUE)
    ...
    tmp <- quanteda:::resample(1:10, group = grvec, size = c(1, 1, 3), replace = TRUE)
    ...
    tmp <- quanteda:::resample(1:10, group = grvec, replace = TRUE)
    ...
    expect_error(
        quanteda:::resample(1:10, group = grvec[1:6], size = c(2, 3), replace = TRUE),
        "group not equal in length of x"
    )
    expect_error(
        quanteda:::resample(1:10, group = grvec, size = c(2, 3), replace = TRUE),
...
test_that("resample works with groups of size 1", {
    ...
    for (i in seq_len(10)) {
        tmp <- quanteda:::resample(1:4, group = grvec, replace = TRUE)
