...
test_that("regex2fixed converts regex patterns correctly", {
    ...
    expect_identical(setdiff(
        regex2fixed5(regex, types, 'fixed', case_insensitive=TRUE),
        list('C', 'c')
    ), list())
    
    expect_identical(setdiff(
        regex2fixed5(regex, types, 'fixed', case_insensitive=FALSE),
        list('c')
    ), list())
    ...
    expect_identical(setdiff(
        regex2fixed5(regex, types, 'regex', case_insensitive=FALSE),
