...
test_that("dfm print works with options as expected", {
    ...
    expect_output(
        ...
        paste0("^Document-feature matrix of: 14 documents, 4,452 features \\(81\\.97% sparse\\)\nand 4 docvars",
               ".*",
               "\\[ reached max_ndoc \\.\\.\\. 8 more documents, reached max_nfeat \\.\\.\\. 4,442\nmore features \\]$")
    )
    expect_output(
...
test_that("test empty dfm is handled properly (#1419)", {
    ...
    expect_output(print(mx),
                  "Document-feature matrix of: 6 documents, 0 features (0.00% sparse) and 1\ndocvar.", fixed = TRUE)
})
