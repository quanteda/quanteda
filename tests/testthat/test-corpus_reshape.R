test_that("corpus_reshape works for sentences", {
    ...
    corp <- corpus(c(textone = "This is a sentence.  Another sentence.  Yet another.", 
                     ...
                     textthree = ""), 
                   docvars = data.frame(country = factor(c("UK", "US", "ZA")), 
                                        year=c(1990, 2000, 2020)))
    corp_rs <- corpus_reshape(corp, to = "sentences")
    expect_equal(docid(corp_rs),
                 factor(c("textone", "textone", "textone", "texttwo", "texttwo", "textthree"),
                        levels = c("textone", "texttwo", "textthree")))
    expect_equal(as.character(corp_rs)[4], c(texttwo.1 = "Premiere phrase."))
    expect_equal(as.character(corp_rs)[6], c(textthree.1 = ""))
    expect_equal(docvars(corp_rs, "country"), 
                 factor(c("UK", "UK", "UK", "US", "US", "ZA"), levels = c("UK", "US", "ZA")))
    
    expect_message(
        corpus_reshape(corp, verbose = TRUE),
...
test_that("corpus_reshape works for paragraphs", {
    ...
    corp_rs <- corpus_reshape(corp, to = "paragraphs")
    
    expect_equal(as.character(corp_rs)[4],
                 c(d2.2 = "No there is another."))
    
    expect_message(
        corpus_reshape(corp, to = "paragraphs", verbose = TRUE),
...
test_that("corpus_reshape works to sentences and back", {
    ...
    corp_rs <- corpus_reshape(corp, to = "sentences")
    expect_error(
        corpus_reshape(corp_rs, to = "paragraphs"),
        "reshape to paragraphs only goes from documents or segments"
    )
    corp_unshaped <- corpus_reshape(corp_rs, to = "documents")
...
test_that("corpus_reshape works with segmented corpus", {
    ...
    corp_rs <- corpus_reshape(corp_segmented, to = "sentences")
    corp_rs <- corpus_reshape(corp_rs, to = "documents")
    expect_equal(as.character(corp_rs),
...
test_that("corpus_reshape preserve empty documents (#1978)", {
    ...
    expect_identical(
        docnames(corp),
        docnames(corpus_reshape(corp_rs, to = "documents"))
    )
})
