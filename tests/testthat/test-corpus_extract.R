...
test_that("corpus_extracttags is working with fixed", {
    ...
    corp <- corpus(c(d1 = "##TAG One",
                     d2 = "##TAG Two"))
    corp_ext1 <- corpus_extracttags(corp, '##TAG', valuetype = 'fixed', pattern_position = 'before')
    expect_equal(texts(corp_ext1), c("d1.1" = "One",
                                     "d2.1" = "Two"))
    expect_equal(docvars(corp_ext1, 'pattern'), c('##TAG', '##TAG'))
    
    # corp_ext1 <- corpus_extracttags(corp, pattern = '##TAG', valuetype = 'fixed', pattern_remove = FALSE)
...
test_that("corpus_extracttags is working with glob", {
    ...
    corp_ext1 <- corpus_extracttags(corp, '##*', valuetype = 'glob', pattern_position = 'before')
    expect_equal(texts(corp_ext1), c("d1.1" = "One",
                                     "d2.1" = "Two"))
    expect_equal(docvars(corp_ext1, 'pattern'), c('##TEST1', '##TEST2'))
...
test_that("corpus_extracttags is working with regex", {
    ...
    corp_ext1 <- corpus_extracttags(corp, '##[A-Z0-9]+', valuetype = 'regex', pattern_position = 'before')
    expect_equal(texts(corp_ext1), c("d1.1" = "One",
                                     "d2.1" = "Two"))
    expect_equal(docvars(corp_ext1, 'pattern'), c('##TEST1', '##TEST2'))
