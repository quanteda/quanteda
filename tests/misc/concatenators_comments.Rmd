...
attr(tokens_concatenate(tokens_ngrams(toks, n = 2)), 'ngrams') # 1
attr(tokens_concatenate(tokens_ngrams(toks, n = 2), concatenator = '+'), 'types')
# [1] "a+b" "b+c" "c+d" "d+e" "e+g" "g+h" "b+e" "h+i" "i+j"
```
...
toks_uni <- tokens_concatenate(tokens_ngrams(toks, n = 2), concatenator = '+')
...
featnames(dfm_select(dfm(toks_uni), features = 'c d'))     # character(0)
...
```{r, eval=FALSE}
toks_uni2 <- tokens_concatenate(tokens_ngrams(toks, n = 2), concatenator = ' ')
...
# select sequence of bigrams
tokens_select(          toks_bi,  features = list(c('c d', 'e f'))[[1]] # "c d", "e f"
```
...
attr(tokens_concatenate(tokens_compound(toks, coll_bi), concatenator = '_'), 'types') 
#[1] "c"   "d"   "i"   "j"   "a_b" "e_g" "g_h"

attr(tokens_concatenate(tokens_compound(toks, coll_bi), concatenator = ''), 'types') 
#[1] "c"   "d"   "i"   "j"   "ab" "eg" "gh"
...
attr(tokens_concatenate(toks_comp, concatenator = '_'), 'ngrams') # 1
```
...
```
Again, if users concatenate compounds with a whitespace, `dfm_select()` and `tokens_select()` will behave differently, but this is an intential act.

```{r, eval=FALSE}
toks_comp2 <- tokens_concatenate(tokens_compound(toks, coll_bi), concatenator = ' ')
