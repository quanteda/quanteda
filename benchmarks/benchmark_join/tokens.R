...
seqs_not <- list(c('not', '*'))
microbenchmark::microbenchmark(
    #joinTokens(toks, seqs_not, valuetype='glob', verbose=TRUE), # too unstable
    joinTokens2(toks, seqs_not, valuetype='glob', verbose=TRUE),
    times=10
)

seqs_will <- list(c('will', '*'))
microbenchmark::microbenchmark(
    #joinTokens(toks, seqs_will, valuetype='glob', verbose=TRUE), # too unstable
    joinTokens2(toks, seqs_will, valuetype='glob', verbose=TRUE),
    times=10
)
...
#     joinTokens2(tokens(inaugCorpus), seq_lex, valuetype='glob', verbose=TRUE),
#     times=1
# )

profvis::profvis(joinTokens2(tokens(inaugCorpus), seq_lex, valuetype='glob', verbose=TRUE))
