...
*This vignette serves as a brief introduction to the core features of *quanteda*. For a comprehensive guide to quantitative text analysis in R, see the (work-in-progress) book [here](http://kbenoit.github.io/quanteda)*
...
A corpus contains a set of texts, variables associated with each document, and metadata associated with the corpus as a whole. Metadata can be used to store information such as the source of the documents, or to package supplementary material with a corpus. For example, if the corpus analysis is part of a model
that includes other forms of data, they can be included here.

A corpus also contains `settings`, which record various options for the preprocessing and feature- extraction procedures provided by the package.

To try the functions provided for interacting with corpora, load the `inaugCorpus` object packaged with `quanteda`. This corpus contains US presidents' inaugural addresses since 1789, with document-level variables for the year of each address (`Year`) and the last name of the president (`President`).

```{r}
# make sure quanteda is loaded
library(quanteda)
...
The `docvars` function returns the document-level variables. This is a *replacement function*[^replacement], which means that as well as retrieving the document variables, we can also set new ones.

[todo: replacement function example]

[^replacement](link to explanation)
