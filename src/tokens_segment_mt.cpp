...
Targets segment(Text tokens,
    ...
    size_t last = segments.size() - 1;
    // Rcout << "last " << last << "\n";
    // Rcout << "segments[last] " << segments[last].second << "\n";
    if (segments[last].second < tokens.size() - 1) {
        segments.push_back(make_pair(segments[last].second + 1, tokens.size() - 1));
    }
...
struct segment_mt : public Worker{
    ...
    segment_mt(Texts &texts_, std::vector<Targets> &temp_,
            const std::vector<std::size_t> &spans_, const SetNgrams &set_patterns_):
            texts(texts_), temp(temp_), spans(spans_), set_patterns(set_patterns_){}
    
    // parallelFor calles this function with size_t
    void operator()(std::size_t begin, std::size_t end){
        //Rcout << "Range " << begin << " " << end << "\n";
        for (std::size_t h = begin; h < end; h++){
            temp[h] = segment(texts[h], spans, set_patterns);
        }
    }
};
...
#if QUANTEDA_USE_TBB
    segment_mt segment_mt(texts, temp, spans, set_patterns);
...
qatd_cpp_tokens_segment(toks, letters, list(c(5, 6), 10))
