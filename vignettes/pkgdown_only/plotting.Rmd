...
# Create new document-level variable for each speech
docvars(data_corpus_inaugural, "Year-Speech") <- paste0(docvars(data_corpus_inaugural, "Year"), "-", docvars(data_corpus_inaugural, "President")) 
...
textplot_wordcloud(dfm_trim(dfm_inaug, min_count = 10, verbose = FALSE))
```

You can also plot a "comparison cloud", but this can only be done with fewer than eight documents:

```{r eval=TRUE, fig.width=8, fig.height=8}
dfm_comp <- corpus_subset(data_corpus_inaugural, 
                         President %in% c("Washington", "Jefferson", "Madison")) %>%
    dfm(groups = "Year-Speech", remove = stopwords("english"), remove_punct = TRUE)

textplot_wordcloud(dfm_trim(dfm_comp, min_count = 5, verbose = FALSE), comparison = TRUE)
```
...
freq_grouped <- textstat_frequency(dfm(data_corpus_inaugural_subset), groups = "Year-Speech")
...
rel_freq <- textstat_frequency(dfm_rel_freq, groups = "Year-Speech")
