...
dfmat_inaug_post1990 <- dfm(dfmat_inaug_post1990, remove = stopwords("english"), stem = TRUE, remove_punct = TRUE)
dfmat_inaug_post1990[, 1:5]
```
...
```{r}
topfeatures(dfmat_uk, 20)  # 20 top words
```
...
```{r}
dfmat_inaug_post1992 <- dfm(corp_inaug_post1991, dictionary = dict)
dfmat_inaug_post1992
```
...
tstatdist <- textstat_dist(dfm_weight(dfmat_sotu, scheme = "prop"))
# hiarchical clustering the distance object
pres_cluster <- hclust(tstatdist)
...
```{r}
tstatsim <- textstat_simil(dfmat_sotu, selection = c("fair", "health", "terror"), 
                          method = "cosine", margin = "features")
lapply(as.list(tstatsim), head, 10)
```
