...
if (require(ggplot2))
    ggplot(data = token_info, aes(x = Year, y = Tokens, group = 1)) + geom_line() + geom_point() +
        scale_x_continuous(labels = c(seq(1789, 2017, 12)), breaks = seq(1789, 2017, 12) ) 

# Longest inaugural address: William Henry Harrison
token_info[which.max(token_info$Tokens), ] 
```
...
pertaining to each unit as columns.  We call them "features" rather than terms, because features are more general than terms: they can be defined as raw terms, stemmed terms, the parts of speech of terms, terms after stopwords have been removed,
...
my_dfm_stem <- dfm(my_corpus, remove = stopwords("english"), stem = TRUE, remove_punct = TRUE)
my_dfm_stem[, 1:5]
```
...
set.seed(100)
textplot_wordcloud(my_dfm, min.freq = 6, random.order = FALSE,
                   rot.per = .25, 
...
```{r}
my_dfm_party <- dfm(data_corpus_irishbudget2010, groups = "party", 
                  remove = stopwords("english"), remove_punct = TRUE)
```

We can sort this dfm, and inspect it:
```{r}
dfm_sort(my_dfm_party)[, 1:10]
```
...
```{r}
recent_dfm_dict <- dfm(recent_corpus, dictionary = my_dict)
recent_dfm_dict
```
...
```{r, eval = FALSE}
liwc_dict <- dictionary(file = "~/Dropbox/QUANTESS/dictionaries/LIWC/LIWC2001_English.dic",
                       format = "LIWC")
liwc_dfm <- dfm(data_corpus_inaugural[52:58], dictionary = liwc_dict)
liwc_dfm[, 1:10]
```
...
pres_dist <- textstat_dist(dfm_weight(pres_dfm, "prop"))
# hiarchical clustering the distance object
pres_cluster <- hclust(pres_dist)
...
docnames(data_corpus_irishbudget2010) <- apply(docvars(data_corpus_irishbudget2010, c("name", "party")), 
                1, paste, collapse = " ")
...
quant_dfm <- dfm_trim(quant_dfm, min_count = 4, max_docfreq = 10)
quant_dfm
