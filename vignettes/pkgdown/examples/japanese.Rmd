...
本例で用いる衆議院外務委員会の議事録は，1947年から2017年の間のすべての発言を含んでいる．このコーパスは**quanteda.corpora**を通じてダウンロードできる．
...
# 漢字
kanji_toks <- tokens_select(toks, '^[一-龠]+$', valuetype = 'regex', padding = TRUE)
kanji_col <- textstat_collocations(kanji_toks, min_count = min_count)
toks <- tokens_compound(toks, kanji_col[kanji_col$z > 3,], concatenator = '')

# カタカナ
kana_toks <- tokens_select(toks, '^[ァ-ヶー]+$', valuetype = 'regex', padding = TRUE)
kana_col <- textstat_collocations(kana_toks, min_count = min_count)
toks <- tokens_compound(toks, kana_col[kana_col$z > 3,], concatenator = '')

# 漢字，カタカナおよび数字
any_toks <- tokens_select(toks, '^[０-９ァ-ヶー一-龠]+$', valuetype = 'regex', padding = TRUE)
...
speec_dfm <- 
    ...
    dfm_trim(min_count = 10, max_count = 0.1)
```
...
```{r}
key <- textstat_keyness(speec_dfm, docvars(speec_dfm, 'year') >= 2001)
...
```{r}
key <- key[!str_detect(rownames(key), regex('委員|大臣')),]
head(key, 20) %>% knitr::kable()
```
...
textplot_network(speech_fcm, min_freq = 0.95, edge_alpha = 0.9, vertex_size = size / max(size) * 3)
```
