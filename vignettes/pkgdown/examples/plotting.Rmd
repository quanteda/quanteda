...
data_corpus_inaugural_subset$Year_President <- paste(
    data_corpus_inaugural_subset$Year, 
    data_corpus_inaugural_subset$President, sep = " ")

# Get frequency grouped by speech
tstat_freq_grouped <- data_corpus_inaugural_subset |> 
    tokens() |> 
    dfm() |> 
    textstat_frequency(groups = Year_President)

# Filter the term "american"
freq_american <- subset(tstat_freq_grouped, feature %in% "american")  
...
```{r eval=TRUE, fig.width=8, fig.height=4}
dfmat_rel_freq <- data_corpus_inaugural_subset |> 
    tokens() |> 
    dfm() |> 
    dfm_weight(scheme = "prop") * 100
print(dfmat_rel_freq)

dat_rel_freq <- textstat_frequency(dfmat_rel_freq, groups = Year_President)

# Filter the term "american"
dat_rel_freq_american <- subset(dat_rel_freq, feature %in% "american")  

ggplot(dat_rel_freq_american, aes(x = frequency, y = group)) +
    geom_point() + 
    scale_x_continuous(limits = c(0, 0.7), breaks = c(seq(0, 0.7, 0.1))) +
        labs(x = "Relative frequency",
         title = 'Relative frequency of "american"')
```
