...
本コーパスは，***によって2017年総選挙の際に収集されたツイートを含んでいる．インターネット上で収集されたテキストは，英数字の全半角が統一されていないので，`stri_trans_nfkc()`によって表記を標準化すると良い．
...
# 漢字
kanji_toks <- tokens_keep(toks, '^[一-龠]+$', valuetype = 'regex', padding = TRUE)
kanji_col <- textstat_collocations(toks, min_count = min_count, size = 2)
toks <- tokens_compound(toks, kanji_col[kanji_col$z > 3,], concatenator = '')

# カタカナ
kana_toks <- tokens_keep(toks, '^[ァ-ヶー]+$', valuetype = 'regex', padding = TRUE)
kana_col <- textstat_collocations(toks, min_count = min_count, size = 2)
kana_toks <- tokens_compound(toks, col_kana[col_kana$z > 3,], concatenator = '')

# 漢字，カタカナおよび数字
any_toks <- tokens_select(toks, '^[0-9ァ-ヶー一-龠]+$', valuetype = 'regex', padding = TRUE)
any_col <- textstat_collocations(any_toks, min_count = min_count)
toks <- tokens_compound(toks, any_col[any_col$z > 3,], concatenator = '')

# ハッシュと語の結合
toks <- tokens_compound(toks, phrase('# *')) 
...
ここでは，`dfm_select()`でハッシュタグのみを選択しているが，クラスターを明確にするために，ツイートの多くに含まれる'#選挙'や'#衆院選'などのハッシュタグは削除してある．

```{r}
tag_fcm <- 
    tweet_dfm %>% 
    dfm_select('#*') %>% 
    dfm_remove(c('*選挙*', '*衆院選*', '*衆議院*', '*投票*')) %>% 
