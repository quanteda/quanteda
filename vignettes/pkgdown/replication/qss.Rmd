...
docvars(corpus_raw, "paper_numeric") <- 1:ndoc(corpus_raw)
...
docvars(corpus_raw, "author") <- ifelse(docvars(corpus_raw, "paper_numeric") %in% hamilton, "Hamilton",
                                       ifelse(docvars(corpus_raw, "paper_numeric") %in% madison, "Madison", 
                                              "Unknown"))
```
...
set.seed(100)
textplot_wordcloud(dfm_papers["No. 12", ], max.words = 20)
  
textplot_wordcloud(dfm_papers["No. 24", ], max.words = 20)
```
...
author_data_known <- subset(author_data, author %in% c("Hamilton", "Madison"))
