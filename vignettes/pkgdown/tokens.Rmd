...
In **quanteda** version 4.0, we introduced a new class of tokens object, `tokens_xptr`. This object enhances the efficiency of adjusting tokens objects based on large text corpora. It is a sub-class of `tokens` object that is developed based on the external-pointer object `Rcpp::XPtr`.

The package's `tokens` object has been a list of vectors that record integer IDs of tokens, which is converted to a C++ object, modified and converted back to R every time `tokens_*` functions are applied. 
Such conversion between R and C++ takes much of the execution time as the size of the object increases.

However, the `tokens_xptr` object keeps the data as the C++ object, eliminating the transaction cost. If the new object is used, large tokens objects (more than one million tokens) can be processed through a complex pipeline very fast. A detailed comparison of performance gains and execution times for `tokens` and `tokens_xptr` objects is available in a [separate vignette](./articles/tokens.html). 
