...
**quanteda** version 4.0 can process textual data significantly faster than its earlier versions thanks to the [tokens_xptr object](./articles/tokens.html) and a new glob pattern matching mechanism. We created the **quanteda3** package from the version 3.2 and compared the version 4.0 with it on a Windows laptop with AMD Ryzen 7 PRO processor (8 cores). We used sentences from 10,000 English-language news articles in this benchmarking.

We repeat the same operation using the different versions of the same function to get the distribution of execution time. The result shows that the execution time of many v4.0 functions is about the half of the version 3.2 functions. 
...
# remove stop words
microbenchmark::microbenchmark(
    v3 = quanteda3::tokens_remove(toks, stopwords("en"), padding = TRUE),
    v4 = as.tokens_xptr(xtoks) %>% 
