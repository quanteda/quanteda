...
Although v4.0 tokenizer is more flexible, its speed is roughly the same that of the v3.2 tokenizer. The shorter execution time of the version 4.0 is due to the faster removal of punctuation marks, numbers and symbols. We compare the performance by tokenizing the corpus with the `tokens()` function from **quanteda**  version 3.2 (upper part of figure below) and version 4.0 (lower part). We repeat the tokenization for each procedure ten times.

```{r}
# compare default tokenization of quanteda version 4.0 with version 3.2
...
We also compare the speed of creating a document-feature matrix based on the tokens object from **quanteda** version 3.2 and the new `as.tokens_xptr()` function implemented in version 4.0.
