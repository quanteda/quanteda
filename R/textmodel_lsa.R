...
#' @export
predict.textmodel_lsa_fitted <- function(LSAspace, newdata = NULL, ...) {
    
    call <- match.call()
    if (is.null(newdata)) newdata <- LSAspace$data
    
    tsa =  newdata %*% LSAspace$features %*% solve(diag(LSAspace$sk))
