...
#' @export
statLexdiv <- function(x, ...) {
    UseMethod("statLexdiv")
}
    
    
#' @rdname statLexdiv
#' @details \code{statLexdiv} calculates a variety of proposed indices for 
...
#'   Tweedie & Baayen, 1998): \deqn{U = \frac{(\lg{N})^2}{\lg{N} - \lg{V}}}{U = 
#'   lg(N)^2 / lg(N) - lg(V)}}
#'   
#'   \item{\code{"S"}:}{Summer's index: \deqn{S = 
#'   \frac{\lg{\lg{V}}}{\lg{\lg{N}}}}{S = lg(lg(V)) / lg(lg(N))}}
#'   
#'   \item{\code{"K"}:}{Yule's
...
#'   
#'   \item{\code{"Maas"}:}{Maas' indices (\eqn{a}, \eqn{\lg{V_0}} & 
#'   \eqn{\lg{}_{e}{V_0}}): \deqn{a^2 = \frac{\lg{N} - \lg{V}}{\lg{N}^2}}{a^2 = 
#'   lg(N) - lg(V) / lg(N)^2} \deqn{\lg{V_0} = \frac{\lg{V}}{\sqrt{1 - 
#'   \frac{\lg{V}}{\lg{N}}^2}}}{lg(V0) = lg(V) / sqrt(1 - (lg(V) / lg(N)^2))} 
#'   The measure was derived from a formula by M\"uller (1969, as cited
#'   in Maas, 1972). \eqn{\lg{}_{e}{V_0}} is equivalent to \eqn{\lg{V_0}}, only 
#'   with \eqn{e} as the base for the logarithms. Also calculated are \eqn{a}, 
#'   \eqn{\lg{V_0}} (both not the same as before) and \eqn{V'} as measures of 
...
#' @rdname statLexdiv
#' @export
statLexdiv.numeric <- function(x, measure=c("TTR", "C", "R", "CTTR", "U", "S", "Maas"), 
                               log.base=10, ...) {
