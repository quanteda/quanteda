...
#' @title Virtual class "cfm4" for a context-feature matrix
#' 
#' @description The cfm4 class of object is a special type of \link{dfm-class}.
#'   
...
#' @import Matrix
#' @export
#' @details The funciton \link{cfm4} provides a very general implementation of a 
...
#'   
#'   \link{cfm4} provides all of this functionality, returning a \eqn{V * V} matrix
#'   (where \eqn{V} is the vocabulary size, returned by \code{\link{ntype}}). The \code{tri = TRUE} option will only return the upper
#'   part of the matrix.
#'   
#'   Unlike some implementations of co-occurrences, \link{cfm4} counts feature co-occurrences 
#'   with themselves, meaning that the diagonal will not be zero.
#'   
#'   \link{cfm4} also provides "boolean" counting within the context of "window", which differs from the counting within "document".  
...
#'   with Dense Vectors.}
#'   
#'   Chruch, K.W. & Hanks, P. (1990)  "\href{http://dl.acm.org/citation.cfm4?id=89095}{Word 
...
#' cfm4(txt, context = "window", window = 2)
#' cfm4(txt, context = "window", count = "weighted", window = 3)
#' cfm4(txt, context = "window", count = "weighted", window = 3, weights = c(3,2,1), ordered = TRUE, tri = FALSE)
#' 
#' # with multiple documents
#' txts <- c("a a a b b c", "a a c e", "a c e f g")
#' cfm4(txts, context = "document", count = "frequency")
#' cfm4(txts, context = "document", count = "boolean")
#' cfm4(txts, context = "window", count = "boolean", window = 2)
#' 
#' txt <- c("The quick brown fox jumped over the lazy dog.",
#'          "The dog jumped and ate the fox.")
#' toks <- tokenize(toLower(txt), removePunct = TRUE)
#' cfm4(toks, context = "document")
#' cfm4(toks, context = "window", window = 3)
#' @import data.table
#' @import Matrix
#' @export
cfm4.tokenizedTexts <- function(x, context = c("document", "window"), 
  ...
  if (context == "window") { 
    ...
    n <- sum(lengths(numericX)) * (window + 1)
    
    result <- fcm_cpp4(numericX, length(types), count, window, weights, ordered, n)
  ...
  result <- new("cfm4", as(result, "dgCMatrix"), count = count,
...
#' @rdname cfm4
#' @export
cfm4.corpus <- function(x, ...) {
  cfm4(texts(x), ...)
}

#' @rdname cfm4
#' @export
cfm4.character <- function(x, ...) {
  cfm4(tokenize(x), ...)
}
...
#' @rdname print.dfm
#' @export
setMethod("print", signature(x = "cfm4"), 
...
#' @rdname print.dfm
setMethod("show", signature(object = "cfm4"), function(object) print(object))

#' @export
cfm2.indexedTexts <- function(x, context = c("document", "window"), 
                                count = c("frequency", "boolean", "weighted"),
                                window = 5L,
                                weights = 1L,
                                ordered = FALSE,
