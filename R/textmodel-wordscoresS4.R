...
#' ws <- textmodel2(LBGexample, c(seq(-1.5, 1.5, .75), NA), model="wordscores")
#' ws
#' predict(ws)
#' # same as:
#' ws2 <- textmodel2_wordscores(LBGexample, c(seq(-1.5, 1.5, .75), NA))
#' print(ws2)
#' wsp2 <- predict(ws2)
#' print(wsp2)
...
#' @export
textmodel2_wordscores <- function(data, scores,
...
#' @rdname textmodel2_wordscores
#' @param object the fitted wordscores textmodel on which prediction will be made
...
#' @author Ken Benoit, borrowed in places from Will Lowe, who probably borrowed 
#'   from me at some early stage.
#' @export
...
#' @rdname textmodel2_wordscores
#' @export
show.textmodel_wordscores_fitted <- print.textmodel_wordscores_fitted
