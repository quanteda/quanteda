...
#' @param sort If TRUE (the default), order points from low to high score. 
#'             If a vector, order according to these values from low to high.
#'             Only applies when \code{type} is 'docs'
#' @param doclabels a vector of names for document. If left NULL (the default), ordinary 
#'                  document names will be used.
#' @param left.mar an overridden left margin, passed to \code{par} (default 8.1). This
#'                 overrides R's default 4.1, which is typically too cramped for document names.
#' @param pickout A vector of feature names to draw attention to in a feature plot. Only 
#'                applies if \code{type} is 'feats'
#' @param alpha A number between 0 and 1 (default 0.5) representing the level of alpha
#'              transparency used to overplot feature names in a feature plot. 
#'              Only applies if \code{type} is 'feats'.
...
plot.textmodel_wordfish_fitted <- function(x, type = c("docs", "feats"), doclabels = NULL,
  ...
  if (type == "docs"){
    ...
    text(x@beta, x@psi, labels = x@features, col = rgb(0.7, 0.7, 0.7, alpha))
