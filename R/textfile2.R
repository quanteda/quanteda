...
setMethod("textfile2", 
          ...
          definition = function(file, textField, directory=NULL, ...) {
              ...
              if (fileType == "csv") {
                  ...
                  sources <- get_csv2(file, textField, ...) 
              } else if (fileType == "tab") {
                  if (length(textField) != 1)
                      stop("textField must be a single field name or column number identifying the texts.")
                  sources <- get_csv2(file, textField, sep = "\t", ...) 
                  ...
                  # cat("file = ", file)
                  sources <- get_csvs2(file, textField, ...)
              } else {
...
## csv format
get_csv2 <- function(file, textField, sep=",", ...) {
...
## csv format multiple
get_csvs2 <- function(filemask, textField, sep=",", ...) {
    print("calling csv")
    ...
    docv <- NULL
    for (f in filenames) {
        thisdocv <- readr::read_delim(f,  delim=sep, quote = "\"")
        if (is.character(textField)) {
...
# }
get_txts2 <- function(filemask, textnames=NULL, ...) {
    # get the pattern at the end
    pattern <- getRootFileNames(filemask)
    # get the directory name
    path <- substr(filemask, 1, nchar(filemask) - nchar(pattern))
